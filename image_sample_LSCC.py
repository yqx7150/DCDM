...
def main():
    ...
    logger.configure(dir="/home/lqg/Desktop/HR/LDPET/LSCC_sample/conver")
    ...
    for step, batchNcond in enumerate(data):
        ...
        try:
            ...
            print("psnr norm")
            psnr = compare_psnr(
                np.pad(maxminNorm(sample.detach().cpu().numpy()), pad_width, mode='constant', constant_values=0),
                np.pad(maxminNorm(batch.detach().cpu().numpy()), pad_width, mode='constant', constant_values=0)
            )
        finally:
            print("psnr norm")
            psnr = compare_psnr(
        ...
        print(sample.detach().squeeze(0).permute(1, 2, 0).cpu().numpy().shape)
        # plt.imshow(sample.detach().squeeze(0).permute(1, 2, 0).cpu().numpy())
